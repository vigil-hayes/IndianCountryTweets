...
def setUpLogs():
	...
	try:
		hour = datetime.datetime.strftime(datetime.datetime.now(), '%H')
                ...
                if next_hour == 24:
                        next_hour=1
		logfile = "logs/%s/%s.json" % (datetime.datetime.strftime(datetime.datetime.now(), '%Y-%m-%d'), 'actors_%s' % next_hour)
		return logfile
	except Exception as e:
		logging.basicConfig(format='%(levelname)s:%(message)s', level=logging.DEBUG)
...
def getActors(userfile):
	try:
		actors = []
		with open(userfile, 'r') as acf:
			users = json.load(acf)
			for user in users:
				uid = str(user['id'])
				actors.append(uid)
...
if __name__ == '__main__':
    ...
    try:
	# Get actors 
	actors=getActors(userfile)
	stream.filter(follow=actors)
